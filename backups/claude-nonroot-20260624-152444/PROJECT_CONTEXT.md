...
- **Binario Claude Code**: base `claude`; en dev se fuerza el wrapper rápido `/usr/local/bin/claude-fast` vía `CLAUDE_CODE_BIN`
...
- **2026-06-23**: Claude Code en `root` podía quedar lento o colgado por estado corrupto del perfil `~/.claude`/`~/.claude.json` tras reinstalar o reauth. Mitigación estable: rotar el perfil a backup, recrear un perfil mínimo, usar `/usr/local/bin/claude-fast` en dev y mantener `.claudeignore` para excluir artefactos pesados del repo.
