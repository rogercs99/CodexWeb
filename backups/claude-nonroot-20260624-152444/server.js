  ...
  try {
    ...
    if (chatRuntime.activeAgentId === 'claude-code') {
            ...
            if (msgType === 'assistant') {
              ...
              for (const block of contentArr) {
                ...
                if (block && block.type === 'thinking' && typeof block.thinking === 'string' && block.thinking) {
                  sendSse(res, 'reasoning_delta', { itemId: 'claude_thinking', text: block.thinking });
                }
              }
              if (fullMsgText) {
                const prevSnap = claudePerMsgSnapshots.get(messageId) || '';
                claudePerMsgSnapshots.set(messageId, fullMsgText);
                const trueDelta = fullMsgText.startsWith(prevSnap)
                  ? fullMsgText.slice(prevSnap.length)
                  : fullMsgText;
                if (trueDelta) {
                  claudeAssistantOutput += trueDelta;
                  claudeHasStreamedDelta = true;
                  sendSse(res, 'assistant_delta', { text: trueDelta });
              ...
              const toolInput = parsed.input ? JSON.stringify(parsed.input).slice(0, 200) : '';
              sendSse(res, 'reasoning_delta', { itemId: 'claude_tool', text: `[${toolName}] ${toolInput}\n` });
            } else if (msgType === 'system' || msgType === 'result') {
              const subtype = String(parsed.subtype || '');
              if (subtype === 'success') {
                const result = String(parsed.result || '').trim();
                if (result) {
                  claudeAssistantOutput = result;
                }
              }
            }
          } catch (_parseError) {
      ...
      claudeProcess.on('close', (code, signal) => {
        ...
        let success = normalizedExitCode === 0 && !runWasKilled;
        ...
        if (!success && !runWasKilled) {
          ...
          if (hasAuthError) {
            ...
            output = cleanStderr || `Claude Code terminó con código ${normalizedExitCode}${signal ? ` (señal ${signal})` : ''}.`;
          }
